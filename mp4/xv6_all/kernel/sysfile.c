...
create(char *path, short type, short major, short minor)
{
  ...
  if((ip = dirlookup(dp, name, 0)) != 0){
    ...
    if(type == T_FILE && (ip->type == T_FILE || ip->type == T_DEVICE)){
      // printf("create 1return ip\n");
      return ip;
    }
    iunlockput(ip);
    // printf("create 2return 0\n");
    return 0;
  }
...
sys_open(void)
{
  ...
  if(ip->type == T_SYMLINK){
    ...
    if(!(omode & O_NOFOLLOW)){
      int cnt = 0;
      while(cnt < 12 && ip->type == T_SYMLINK){
...
sys_chdir(void)
{
  ...
  if(ip->type == T_SYMLINK){
    ...
    int cnt = 0;
    while(cnt < 12 && ip->type == T_SYMLINK){
      //printf("cnt in chdir_while = %d\n", cnt);
      readi(ip, 0, (uint64)target, 0, MAXPATH);
      //printf("chdir_target = %s\n", target);
      iunlockput(ip);
      if((ip = namei(target)) == 0){
        //printf("ip = %d\n", ip);
        //printf("return -1");
