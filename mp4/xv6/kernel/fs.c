...
namex(char *path, int nameiparent, char *name)
{
  ...
  if(*path == '/')
    ...
    ip = idup(myproc()->cwd);
  //printf("namex\n");
  //printf("path = %s  type = %d  inum = %d\n", path, ip->type, ip->inum);
  while((path = skipelem(path, name)) != 0){ 
    ilock(ip);
    //printf("in_while: path = %s  type = %d  inum = %d\n", path, ip->type, ip->inum);
    ////printf("ip->type = %d\n", ip->type);
    //int count = 0;
    while(ip->type == T_SYMLINK && cnt < 12){
      //count++;
      cnt++;
